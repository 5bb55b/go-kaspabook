...
////////////////////////////////
var rocks *C.rocksdb_transactiondb_t
var cfHandleList []*C.rocksdb_column_family_handle_t
...
func initRocks() {
    ...
    if config.Rocksdb.DtlIndex > 0 {
        cFilterList[cfIndex] = C.rocks_newCompactionFilter(C.uint64_t(config.Rocksdb.DtlIndex))
    ...
    dbNameC := C.CString(config.Rocksdb.Path)
    cfHandleList = make([]*C.rocksdb_column_family_handle_t, lenCF)
    defer func() {
    ...
    var errC *C.char
    rocks = C.rocksdb_transactiondb_open_column_families(dbOpt, txDbOpt, dbNameC, C.int(lenCF), &cfNameListC[0], &cfOptList[0], &cfHandleList[0], &errC)
    if errC != nil {
        err := errRocks(errC)
        destroyRocks()
...
func destroyRocks() {
    if rocks != nil {
        C.rocksdb_transactiondb_close(rocks)
        rocks = nil
    }
    if dbOpt != nil {
...
func getCF(tx *C.rocksdb_transaction_t, cf int, key []byte, fGet func([]byte) (error)) ([]byte, error) {
    ...
    var rowData []byte
    if tx != nil {
        rowValC = C.rocksdb_transaction_get_cf(tx, rOpt, cfHandleList[cf], (*C.char)(unsafe.Pointer(&key[0])), C.size_t(lenKey), &lenValC, &errC)
    } else {
        rowValC = C.rocksdb_transactiondb_get_cf(rocks, rOpt, cfHandleList[cf], (*C.char)(unsafe.Pointer(&key[0])), C.size_t(lenKey), &lenValC, &errC)
    }
...
func seekCF(tx *C.rocksdb_transaction_t, cf int, keyStart []byte, keyEnd []byte, maxCount int, dsc bool, snapshot *C.rocksdb_snapshot_t, fGet func(int, []byte, []byte) (bool, error)) (error) {
    ...
    if tx != nil {
        iter = C.rocksdb_transaction_create_iterator_cf(tx, rOptSeek, cfHandleList[cf])
    } else {
        iter = C.rocksdb_transactiondb_create_iterator_cf(rocks, rOptSeek, cfHandleList[cf])
    }
    if iter == nil {
...
func putCF(tx *C.rocksdb_transaction_t, cf int, key []byte, val []byte, ds uint64) (error) {
    ...
    var errC *C.char
    if tx != nil {
        C.rocksdb_transaction_put_cf(tx, cfHandleList[cf], (*C.char)(unsafe.Pointer(&key[0])), C.size_t(lenKey), dataC, C.size_t(lenVal), &errC)
    } else {
        C.rocksdb_transactiondb_put_cf(rocks, wOpt, cfHandleList[cf], (*C.char)(unsafe.Pointer(&key[0])), C.size_t(lenKey), dataC, C.size_t(lenVal), &errC)
    }
    runtime.KeepAlive(key)
...
func deleteCF(tx *C.rocksdb_transaction_t, cf int, key []byte) (error) {
    ...
    var errC *C.char
    if tx != nil {
        C.rocksdb_transaction_delete_cf(tx, cfHandleList[cf], (*C.char)(unsafe.Pointer(&key[0])), C.size_t(lenKey), &errC)
    } else {
        C.rocksdb_transactiondb_delete_cf(rocks, wOpt, cfHandleList[cf], (*C.char)(unsafe.Pointer(&key[0])), C.size_t(lenKey), &errC)
    }
    runtime.KeepAlive(key)
    if errC != nil {
        return errRocks(errC)
...
func deleteRangeCF(cf int, keyStart []byte, keyEnd []byte) (error) {
    ...
    C.rocksdb_delete_range_cf(dbBase, opt, cfHandleList[cf], (*C.char)(unsafe.Pointer(&keyStart[0])), C.size_t(len(keyStart)), (*C.char)(unsafe.Pointer(&keyEnd[0])), C.size_t(len(keyEnd)), &errC)
    runtime.KeepAlive(keyStart)
    runtime.KeepAlive(keyEnd)
    if errC != nil {
        return errRocks(errC)
...
func txBegin() (*C.rocksdb_transaction_t) {
    return C.rocksdb_transaction_begin(rocks, wOpt, txOpt, nil)
}
